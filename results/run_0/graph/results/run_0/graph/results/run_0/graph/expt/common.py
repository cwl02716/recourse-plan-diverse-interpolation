...
from utils.funcs import compute_max_distance, lp_dist, compute_validity, compute_proximity, compute_diversity, compute_distance_manifold, compute_dpp, compute_likelihood, compute_pairwise_cosine, compute_kde, compute_proximity_graph, compute_proximity_graph_
...
def _run_single_instance_plans_graph(idx, method, x0, graph, model, seed, logger, params=dict()):
    ...
    plans, dist, report = method.generate_recourse(x0, model, random_state, params)
    print(plans, dist)
    # print(weighted_adj_matrix.shape, weighted_adj_matrix[1])
    # shortest_path = compute_proximity_graph(weighted_adj_matrix, idx)
    # print(shortest_path)
    # path = compute_proximity_graph_(weighted_adj_matrix, idx)
    # print(path)
    # for i in range(len(path) - 1):
    #     print(weighted_adj_matrix[path[i]][path[i + 1]])
    #     print(weighted_adj_matrix[path[i+1]][path[i]])
